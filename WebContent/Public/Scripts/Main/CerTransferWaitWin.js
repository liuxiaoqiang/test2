Ext.ns("Ynzc");
Ext.ns("Ynzc.manage"); 
Ynzc.manage.CerTransferWaitWin = Ext.extend(Ext.grid.GridPanel, {
    id: 'certransferwaitwin',
    initComponent: function() {
        ...
        var ds = new Ext.data.Store({
            scopte: this,
            url: 'main/carTransfer.html?action=getIntoCarTransferViewList',
            reader: reader,
        ...
        Ext.apply(this, {
            ...
            tbar: [{
                ...
                handler: function() {
            		...
            		if(n==0){
            			Ext.MessageBox.alert("提示","请选中要处理的车辆!");
            			return;
            		}
            		if(n>1){
            			Ext.MessageBox.alert("提示","每次只能处理一辆车!");
            			return;
            		}
            		record=this.selModel.getSelected();
            		if(record.data.transfertype=="管辖区的转移登记"){
//            			Ext.MessageBox.alert("提示","每次只能对业务种类为管辖区外的转移登记的申请进行号牌分配!");
            			Ext.Ajax.request({
                			url:'main/plate.html?action=updateCerTransfer',               			
                			success:function(resp){
                				var result=Ext.util.JSON.decode(resp.responseText);
                				if(result.success==true){  
                					Ext.MessageBox.alert("提示","转移登记成功！");
                				}else{
                					Ext.MessageBox.alert("提示","转移登记失败！");
                				}
                			},
                			params: {
                				  doid:record.data.id
            		...
            		Ext.Ajax.request({
            			url:'main/plate.html?action=getMinPlateCodeByUnitId',           			
            			success:function(resp){
            				var result=Ext.util.JSON.decode(resp.responseText);
            				if(result.success==true){
            					if(result.code=="无号牌"){
            						Ext.MessageBox.alert("提示","号牌库里面没有新号牌！");
            						Ext.MessageBox.alert('系统提示','对不起！号牌库里暂时没有号牌！');
            					}else{
            						Ynzc.manage.codeWin.show();
            						Ext.getCmp("codenum").setValue(result.code);
            						Ynzc.manage.plateCodeInfo = result.code;
            					}
            				}else{
            					Ext.MessageBox.alert("提示","分配号牌失败！");
            				}
            			},
            			failure: function() {
                        ...
                        params: {
                          unitid: record.data.currentunitid,
                          transfertype:record.data.transfertype
                        }
            		});
            		Ynzc.manage.codeWin = new Ext.Window({
            			id: "inputCodeNumWin",
                        title: "车辆新号牌",
                        modal: true,
                        height: 100,
                        ...
                        buttons: [{
                        	text: "确定",
                        	handler: function() {
                        	var cartransferid = record.data.id;
                        	var oldlicense=record.data.oldlicense;
                        	var tractorinfoId=record.data.tractorinfoId;
                        	Ext.Ajax.request({
                    			url:'main/plate.html?action=updatePlateStatus',               			
                    			success:function(resp){
                    				var result=Ext.util.JSON.decode(resp.responseText);
                    				if(result.success==true){  
                    					Ynzc.manage.codeWin.close();
                    					Ext.MessageBox.alert("提示","号牌分配成功！");
                    				}else{
                    					Ext.MessageBox.alert("提示","分配号牌失败！");
                    				}
                    			},
                    			params: {
                    				  tractorinfoId:tractorinfoId,
                    				  plateCode: Ynzc.manage.plateCodeInfo,
                    				  oldlicense: oldlicense,
                    				  auditcartransferid: cartransferid,
                    				  currentregionid: record.data.currentregionid,
                    				  orginregionid:record.data.orginregionid
                    			}
                    		});
                ...
                handler: function() {
                    new Ext.Window({
                        ...
                        buttons: [{
                            text: "查询",
                            handler: function() {
                        	Ext.getCmp("certransferwaitwin").getStore().reload({
                                    params: {
                                        start: 0,
                                        limit: 20,
                                        state: "0,1",
            ...
            handler:function(){
		        		...
		        		var n=selections.length;
	                    if(n==0){
	                        Ext.MessageBox.alert("提示","请选中要打印的转入申请!");
	                        return;
	                    }
	                    if(n>1){
            			...
            			new Ynzc.manage.TracReportWin({
	        					title:'打印拖拉机转入申请表',
	        					sourceURL:'main/carTransfer.html?action=tractorInfoTurnInReport',
	        					params:{tractorinfoid:selections[0].data.tractorinfoId,wordType:'tractorRegistrationAndTurnIn'}
	        			}).show();
            		}.createDelegate(this)
            }],
        ...
        Ynzc.manage.CerTransferWaitWin.superclass.initComponent.apply(this, arguments);
        ds.load({
            params: {
                start: 0,
                limit: 20,
...
Ext.reg('certransferwaitwin', Ynzc.manage.CerTransferWaitWin);