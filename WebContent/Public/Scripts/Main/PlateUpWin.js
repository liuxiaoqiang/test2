Ext.ns("Ynzc");
Ext.ns("Ynzc.manage");
Ynzc.manage.PlateUpWin=Ext.extend(Ext.grid.GridPanel,{
                id:'plateupwin',
...
//                                                        new Ynzc.manage.PlateSubmitWin().show(Ext.getCmp("addPlateBtn").getEl());
                        	...
                        	id:"addPlateBtn",
                        	text:"查看<font color=red>/</font>修改号牌需求",
							iconCls:"icon-search",
                        	handler:function(){
...
//                                }
                        	...
                        	handler:function(){
                                ...
                                for(i=0;i<n;i++){
                                	if(selections[i].data.istrue==0){
                                      ...
                                      Ext.MessageBox.alert("提示","部分记录已确认不能删除!");
                                      return;  
                                    }
                                }
                            	Ext.Msg.show({
                        	...
                        	handler:function(){
                                ...
                                for(i=0;i<n;i++){
                                    if(selections[i].data.istrue==0){
                                      ...
                                      Ext.MessageBox.alert("提示","部分记录已上报或处理完成,不会重复上报!");
                                    }
                                }
	                ...
	                mask.show();
                                                Ext.Ajax.request({
                                                    url:"main/plate.html?action=Submit",
                                                    method:"post",
                                                    success:function(){
                                                    mask.hide();
                                                        Ext.MessageBox.alert("提示","已成功上报选中的批次");
                                                        ds.reload();
                                                    },
                                                    faliure:function(){
                                                        Ext.Msg.alert("警告","<font color=red>与服务器通信失败!</font>");
...
//                        }
                    ...
                    Ynzc.manage.PlateUpWin.superclass.initComponent.apply(this,arguments);
                    ds.load({
                        params:{
                            start:0,
                            limit:20,
...
Ext.reg('plateupwin',Ynzc.manage.PlateUpWin);