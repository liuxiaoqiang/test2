Ext.ns("Ynzc");
Ext.ns("Ynzc.manage"); 
Ynzc.manage.DrivingLicenceChangeGrid=Ext.extend(Ext.grid.GridPanel,{
	id:'drivingLicenceChangeGrid',
	initComponent:function(){
        ...
        var expand= new Ext.ux.grid.RowExpander({
			tpl : new Ext.XTemplate(
				...
				'<tr><td colspan="5" align="center">驾照变更信息</td></tr>',
				'<tr><td>变更后姓名</td><td>{[values.changeName != null ? values.changeName : ""]}</td><td>变更后生日</td><td colspan="2">{[values.changeBirthday != null ? new Date(values.changeBirthday.time).format("Y-m-d") : ""]}</td></tr>',
				'<tr><td>变更后身份证</td><td>{[values.changeIdCard != null ? values.changeIdCard : ""]}</td><td>变更后家庭地址</td><td colspan="2">{[values.changeAddress != null ? values.changeAddress : ""]}</td></tr>',
				'<tr><td>变更后联系电话</td><td>{[values.changeLinkTel != null ? values.changeLinkTel : ""]}</td><td colspan="3">&nbsp;</td></tr>',
	    ...
	    var reader=new Ext.data.JsonReader({
	        ...
	        fields:[{
	            ...
	            name : 'auditingChangeInfoFlag'
	        },{
	            name : 'auditingChangeInfoDate'
	        },{
	            name : 'changeLinkTel'
	        },{
        ...
        var ds=new Ext.data.Store({
            scopte:this,
            url:'main/drivingLicenceChange.html?action=getDrivingLicenceChange',
            reader:reader,
            sortInfo:{
                field:'id',
                direction:'DESC'
        ...
        Ext.apply(this,{
        	...
        	tbar:[{
        		...
        		iconCls:"icon-Add",
        		handler:function(){
	        		Ynzc.manage.addDriverwin=new Ynzc.manage.DrivingLicenceChangeWin();
	        		Ynzc.manage.addDriverwin.show();
	        		Ynzc.manage.winName = "drivingLicenceChangeWin";
        		}
        	},'-',{
                text:"业务上报",
                iconCls:"icon-Add",
                    ...
                    for(i=0;i<n;i++){
                     ...
                     Ext.Msg.show({
                        ...
                        fn:function(btn){
                            if(btn=="yes"){
                                Ext.Ajax.request({
                                    url:"main/drivingLicenceChange.html?action=changeState",
                                    method:"post",
            		...
            		new Ynzc.manage.ReportWin(
        				{
        					...
        					params:{id:selections[0].data.drivingLicenceId,peopleInfoId:selections[0].data.peopleInfoId,drivingLicenceTypeId:selections[0].data.drivingLicenceTypeId,operationNameType:selections[0].data.operationNameType,operationId:selections[0].data.id,wordType:'drivingLicence',functionType:'putOut',reloadType:'driverLicenceChangeMgr'}
        				}
        			).show();
            	}.createDelegate(this)
        	},'-',{
                    ...
                    for(i=0;i<n;i++){
                     ...
                     Ext.Msg.show({
                        ...
                        fn:function(btn){
                            if(btn=="yes"){
                                Ext.Ajax.request({
                                    url:"main/drivingLicenceChange.html?action=delDrivingLicenceChange",
                                    method:"post",
                                    success:function(resp){
            							var result=Ext.util.JSON.decode(resp.responseText);
        								if(result.success==true){
        ...
        Ynzc.manage.DrivingLicenceChangeGrid.superclass.initComponent.apply(this,arguments);
        ds.load({
        	params:{
	        	start:0,
	        	limit:20
        ...
        this.on("rowdblclick",function(grid,index,event){
        	...
        	Ynzc.manage.updateDriverwin=new Ynzc.manage.DrivingLicenceChangeUpdateWin();
        	Ynzc.manage.updateDriverwin.show();
            Ynzc.manage.drivingLicenceChangeId=record.data.id;
			Ext.getCmp("changeName").setValue(record.data.changeName);
			Ext.getCmp("changeIdCard").setValue(record.data.changeIdCard);
			Ext.getCmp("changeBirthday").setValue(new Date(record.data.changeBirthday.time).format('Y-m-d'));
			Ext.getCmp("changeAddress").setValue(record.data.changeAddress);
			Ext.getCmp("changeLinkAddress").setValue(record.data.changeLinkAddress);
			Ext.getCmp("changeLinkTel").setValue(record.data.changeLinkTel);
...
Ext.reg('drivingLicenceChangeGrid',Ynzc.manage.DrivingLicenceChangeGrid);