Ext.ns("Ynzc");
Ext.ns("Ynzc.manage");
Ynzc.manage.ReaperPlateMadeWin=Ext.extend(Ext.Window,{
	id:"reaperplatemadewin",
	initComponent:function(){    
		...
		var wtDs=new Ext.data.Store({
            ...
            url:'main/reaperPlate.html?action=getNewPlateListHandle',
            reader:wtReader,
            sortInfo:{
                field:'id',
                direction:'ASC'
		...
		var mdDs=new Ext.data.Store({
            ...
            url:'main/reaperPlate.html?action=getMadePlateList',
            reader:mdReader,
            sortInfo:{
                field:'id',
                direction:'ASC'
			...
			items:[{
				...
				items:[{
					...
					handler:function(){
                        ...
                        if(n==0){
                            Ext.ux.Toast.msg("提示","请选中要制作的号牌!");
                            return;
                        }
                        var rds=[];
                        for(i=0;i<n;i++){
                        	if(selections[i].data.madestatus==3){
                        		Ext.ux.Toast.msg("提示","部分号牌已入库，不能标记");
                        	}else{
                        		rds.push(selections[i].data);
                        	}
                        }
                        Ext.Ajax.request({
                        	url:"main/reaperPlate.html?action=flagMade",
                        	success:function(){
                        		Ext.ux.Toast.msg("提示","标记成功!");
					...
					handler:function(){
                        ...
                        Ext.Ajax.request({
                        	url:"main/reaperPlate.html?action=flagNotMade",
                        	success:function(){
                        		Ext.ux.Toast.msg("提示","标记成功!");
                        		waitGrid.getStore().reload();
                        		madeGrid.getStore().reload();
					...
					handler:function(){
						Ext.Msg.show({
	                            ...
	                            fn:function(btn){
	                                if(btn=="yes"){
	                                    Ext.Ajax.request({
	                                        url:"main/reaperPlate.html?action=flagAll",
	                                        method:"post",
	                                        success:function(){
	                                            Ext.ux.Toast.msg("提示","标记成功");
	                                            waitGrid.getStore().reload();
					...
					handler:function(){
							Ext.Msg.show({
		                        ...
		                        fn:function(btn){
		                            if(btn=="yes"){
		                                Ext.Ajax.request({
		                                    url:"main/reaperPlate.html?action=InputLib",
		                                    method:"post",
		                                    success:function(){
		                                        Ext.ux.Toast.msg("提示","标记成功");
		                                        waitGrid.getStore().reload();
		...
		Ynzc.manage.ReaperPlateMadeWin.superclass.initComponent.apply(this,arguments);
		wtDs.load({
			params:{
				start:0,
				limit:6
