Ext.ns("Ynzc");
Ext.ns("Ynzc.manage"); 
Ynzc.manage.DrivingLicenceTermGrid=Ext.extend(Ext.grid.GridPanel,{
	id:'drivingLicenceTermGrid',
	initComponent:function(){
	    ...
	    var reader=new Ext.data.JsonReader({
	        ...
	        fields:[{
	            ...
	            name : 'auditingTermFlag'
	        },{
	            name : 'auditingTermDate'
	        },{
        ...
        var ds=new Ext.data.Store({
            scopte:this,
            url:'main/drivingLicenceTerm.html?action=getDrivingLicenceTerm',
            reader:reader,
            sortInfo:{
                field:'id',
                direction:'DESC'
        ...
        Ext.apply(this,{
        	...
        	tbar:[{
        		...
        		iconCls:"icon-Add",
        		handler:function(){
	        		Ynzc.manage.addLicencewin=new Ynzc.manage.DrivingLicenceTermWin();
	        		Ynzc.manage.addLicencewin.show();
	        		Ynzc.manage.winName = "drivingLicenceTermWin";
        		}
        	},'-',{
                text:"业务上报",
                iconCls:"icon-Add",
                    ...
                    for(i=0;i<n;i++){
                     ...
                     Ext.Msg.show({
                        ...
                        fn:function(btn){
                            if(btn=="yes"){
                                Ext.Ajax.request({
                                    url:"main/drivingLicenceTerm.html?action=changeState",
                                    method:"post",
            		...
            		new Ynzc.manage.ReportWin(
        				{
        					...
        					params:{id:selections[0].data.drivingLicenceId,peopleInfoId:selections[0].data.peopleInfoId,drivingLicenceTypeId:selections[0].data.drivingLicenceTypeId,operationNameType:selections[0].data.operationNameType,operationId:selections[0].data.id,reloadType:'drivingLicenceTermMgr',functionType:'putOut'}
        				}
        			).show();
        		...
        		handler:function(){
            		...
            		new Ynzc.manage.ReportWin(
        				{
        					title:'打印',
        					sourceURL:'main/reportPeopleInfo.html?action=getPeopleInfoOperationReport',
                    ...
                    for(i=0;i<n;i++){
                     ...
                     Ext.Msg.show({
                        ...
                        fn:function(btn){
                            if(btn=="yes"){
                                Ext.Ajax.request({
                                    url:"main/drivingLicenceTerm.html?action=delDrivingLicenceTerm",
                                    method:"post",
                                    success:function(resp){
            							var result=Ext.util.JSON.decode(resp.responseText);
        								if(result.success==true){
        ...
        Ynzc.manage.DrivingLicenceTermGrid.superclass.initComponent.apply(this,arguments);
        ds.load({
        	params:{
	        	start:0,
	        	limit:20
        ...
        this.on("rowdblclick",function(grid,index,event){
        	...
        	Ynzc.manage.updateLicencewin=new Ynzc.manage.DrivinglicenceTermUpdateWin()
        	Ynzc.manage.updateLicencewin.show();
            Ynzc.manage.bodyInfoId=record.data.bodyInfoId;
			Ext.getCmp("changeHight").setValue(record.data.hight);
			Ext.getCmp("changeResolvingPowerFlag").setValue(record.data.resolvingPowerFlag);
			Ext.getCmp("changeRightEyePwoer").setValue(record.data.rightEyePwoer);
			Ext.getCmp("changeRightEyeRectificationFlag").setValue(record.data.rightEyeRectificationFlag);
			Ext.getCmp("changeLeftEyePwoer").setValue(record.data.leftEyePwoer);
			Ext.getCmp("changeLeftEyeRectificationFlag").setValue(record.data.leftEyeRectificationFlag);
			Ext.getCmp("changeRightEarPwoer").setValue(record.data.rightEarPwoer);
			Ext.getCmp("changeLeftEarPwoer").setValue(record.data.leftEarPwoer);
			Ext.getCmp("changeRightHandFlag").setValue(record.data.rightHandFlag);
			Ext.getCmp("changeLeftHandFlag").setValue(record.data.leftHandFlag);
			Ext.getCmp("changeRightLagFlag").setValue(record.data.rightLagFlag);
			Ext.getCmp("changeLeftLagFlag").setValue(record.data.leftLagFlag);
			Ext.getCmp("changeBodyNeck").setValue(record.data.bodyNeck);
			Ext.getCmp("changeBodyCheckDate").setValue(new Date(record.data.bodyCheckDate.time).format('Y-m-d'));
			Ext.getCmp("changeCheckResultFlag").setValue(record.data.checkResultFlag);
			Ext.getCmp("changeCheckOrganization").setValue(record.data.checkOrganization);
			Ext.getCmp("changeCheckDoctor").setValue(record.data.checkDoctor);
...
Ext.reg('drivingLicenceTermGrid',Ynzc.manage.DrivingLicenceTermGrid);