Ext.ns("Ynzc");
Ext.ns("Ynzc.manage");
Ynzc.manage.UniteHarvesterMoveInfoGrid=Ext.extend(Ext.grid.GridPanel,{
                id:'uniteHarvesterMoveInfo',
                initComponent:function(){
                    ...
                    var cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),sm,{
                        ...
                        dataIndex:"moveType",
                        renderer:function(v){
                         	if(v==0){return "辖区内转移"}else if(v==1){return "转出辖区"}else{return "未知类型"}
                        }
                    },{
                        header:'管辖地',
                        sortable:true,
                        ...
                        header:'登记人',
                        sortable:true,
                        dataIndex:'moveRegisterman'
                    },{
                        header:'登记日期',
                        sortable:true,
                        dataIndex:'moveRegistdate',
                        renderer:function(v){
                    ...
                    var reader=new Ext.data.JsonReader({
                        ...
                        fields:[{
                            ...
                            name : 'moveAcceptunitName'
                        },{
                            name : 'moveApplicationtype'
                        },{
                            name : 'moveRegisterman'
                        },{
                            name : 'moveRegistdate'
                        },{
                            name : 'oldUnitname'
                    ...
                    var ds=new Ext.data.Store({
                        scopte:this,
                        url:'main/uniteharvesterMoveInfo.html?action=getUniteharvesterMoveViewAll',
                        reader:reader,
                        sortInfo:{
                            field:'id',
                            direction:'ASC'
                            ...
                            handler:function(){       
                            	var dengji=new Ext.Window({
                            		...
                            		buttons:[{
                            			...
                            			text:'确定',
                            			handler:function(){
                            				if(checkNum(Ext.getCmp("carnum").getValue())){Ext.ux.Toast.msg("操作提示","需要转移登记的车辆号码不能为空或只能是数字");return};
                            				...
                            				Ext.Ajax.request({
                            					...
                            					success:function(data){
                            						...
                            						if(result!=null&&result.UniteHarvester[0]!=null){
                            							...
                            							var addwin=new Ynzc.manage.UniteHarvesterMoveInfoGridAddWin();
                               					    	addwin.show();
                                ...
                                handler:function(){
                                    ...
                                    if(n==0){
                                        Ext.ux.Toast.msg("提示","请选中要删除的转移登记信息!");
                                        return;
                                    }
                                    var rds=[];
                                    for(i=0;i<n;i++){
                                     ...
                                     Ext.Msg.show({
                                        ...
                                        fn:function(btn){
                                            if(btn=="yes"){
                                                Ext.Ajax.request({
                                                	url:'main/uniteharvesterMoveInfo.html?action=deleteUniteharvesterMove',
                                                    method:"post",
                                                    success:function(){
                                                        Ext.ux.Toast.msg("提示","已成功删除选中的转移登记信息");
                                                        ds.reload();
                                                    },
                                                    faliure:function(){
                                                    ...
                                                    params:{
                                                    	uniteharvestermovelist:Ext.util.JSON.encode(rds)
                                                    }
                                                });
                                            }
                                        }
                    ...
                    Ynzc.manage.UniteHarvesterMoveInfoGrid.superclass.initComponent.apply(this,arguments);
                    ds.load({
                        params:{
                            start:0,
                            limit:20
                    ...
                    this.on("rowdblclick",function(grid,index,event){
                        ...
                        var addwin=new Ynzc.manage.UniteHarvesterMoveInfoGridUpdateWin();
					    	addwin.show();
						    ...
						    Ext.getCmp("applicationType").setValue(record.data.moveApplicationtype);
						    Ext.getCmp("jinbandate").setValue(new Date(record.data.moveRegistdate.time).format('Y-m-d'));
						    Ext.getCmp("jinbanMan").setValue(record.data.moveRegisterman);
						    Ext.getCmp("caraddress").setMyValue(record.data.unitid);
						    Ext.getCmp("unitRegion").setValue(record.data.harvesterGuanXiaDi);
						    Ynzc.manage.uniteHarvestmoveId=record.data.moveid;
						    Ynzc.manage.moveuniteHarvestId=record.data.id;
						    ...
						    if(others[3]==1){Ext.getCmp("xieyizhixing").setValue(true);}
						    if(others[4]==1){Ext.getCmp("gongzhengshu").setValue(true);}
						    var movetype=record.data.moveType;
						    if(movetype==0){Ext.getCmp("inmove").setValue(true)}
						    if(movetype==1){Ext.getCmp("outmove").setValue(true)}
						    Ext.getCmp("combRegion").setValue(record.data.moveAcceptunitName);
						    Ext.getCmp("movenote").setValue(record.data.moveNote);
						    Ynzc.manage.UniteHarvestState=record.data.harvesterState;
						    mask.hide();
                    });
                }
});
Ext.reg('uniteHarvesterMoveInfoGrid',Ynzc.manage.UniteHarvesterMoveInfoGrid);