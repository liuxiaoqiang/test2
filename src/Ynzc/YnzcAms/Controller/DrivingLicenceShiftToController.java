...
public class DrivingLicenceShiftToController extends BaseController{
	
	@SuppressWarnings("unused")
	private final static Logger log = Logger.getLogger(DrivingLicenceShiftToController.class);
	
	public void getDrivingLicenceShiftTo(HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public void addDrivingLicenceShiftTo(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			request.setCharacterEncoding("UTF-8");
			response.setContentType("text/json;charset=utf-8");
			User user=(User)request.getSession().getAttribute("user");
			UnitManage unit = (UnitManage)request.getSession().getAttribute("userUnit");
			DrivingLicenceShiftTo model = new DrivingLicenceShiftTo();
			model.setOperationNameType(6);
			model.setDrivingLicenceId(Integer.parseInt(request.getParameter("drivingLicenceId").trim()));
			model.setGoToAddress("");
			model.setCausation(request.getParameter("causation").trim());
			model.setShiftToAddress(request.getParameter("shiftToAddress").trim());
			...
			model.setState(0);
			if (serviceManager.getDrivingLicenceShiftToService().addDrivingLicenceShiftTo(model)) {
	...
	public void updateDrivingLicenceShiftTo(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			request.setCharacterEncoding("UTF-8");
			response.setContentType("text/json;charset=utf-8");
			User user=(User)request.getSession().getAttribute("user");
			DrivingLicenceShiftTo model = serviceManager.getDrivingLicenceShiftToService().findDrivingLicenceShiftToById(Integer.parseInt(request.getParameter("id").trim()));
			model.setCausation(request.getParameter("causation").trim());
			model.setShiftToAddress(request.getParameter("shiftToAddress").trim());
			model.setProveType(Integer.valueOf(request.getParameter("proveType").trim()));
			model.setProveId(request.getParameter("proveId").trim());
	...
	public void changeState(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			...
			serviceManager.getDrivingLicenceShiftToService().drivingLicenceShiftToState(id,1);
		...
		public void start() {
			timer.schedule(new TimerTask() {
				...
				private void playSound() {
					...
					String[] ids = id.split(",");
					for(String str : ids){
						DrivingLicenceShiftTo model = serviceManager.getDrivingLicenceShiftToService().findDrivingLicenceShiftToById(Integer.valueOf(str));
						if(model.getState()<2){
							model.setAuditingShiftToFlag(1);
							...
							serviceManager.getDrivingLicenceShiftToService().updateDrivingLicenceShiftTo(model);
