...
public class DrivingLicenceLogoutController extends BaseController{

	@SuppressWarnings("unused")
	private final static Logger log = Logger.getLogger(DrivingLicenceLogoutController.class);
	
	public void getDrivingLicenceLogout(HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public void addDrivingLicenceLogout(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			request.setCharacterEncoding("UTF-8");
			response.setContentType("text/json;charset=utf-8");
			User user=(User)request.getSession().getAttribute("user");
			UnitManage unit = (UnitManage)request.getSession().getAttribute("userUnit");
			DrivingLicenceLogout model = new DrivingLicenceLogout();
			model.setOperationNameType(7);
			...
			model.setState(0);
			if (serviceManager.getDrivingLicenceLogoutService().addDrivingLicenceLogout(model)) {
	...
	public void delDrivingLicenceLogout(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			request.setCharacterEncoding("UTF-8");
			response.setContentType("text/json;charset=utf-8");
			String ids = (String) request.getParameter("id").trim();
			boolean isDelete = serviceManager.getDrivingLicenceLogoutService().delDrivingLicenceLogout(ids);
			if (!isDelete) {
	...
	public void changeState(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			...
			serviceManager.getDrivingLicenceLogoutService().drivingLicenceLogoutState(id,1);
